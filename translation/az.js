export default {
  page_not_found: 'Səhifə tapılmadı',
  contact: 'Əlaqə',
  we_make_it:"Biz bunu reallaşdıracağıq", 
  coaching_and_business_studio:"Müasir və yenilikçi Kouçinq və Biznes Məsləhətləri Studiyası",
  we_developed_for:"Tikinti şirkəti üçün hazırladığımız veb sayt.",
  click_to_view:"Digər layihələrə baxmaq üçün klikləyin",
   see_more:"Ətraflı bax",
   services_we_provide:"Təqdim etdiyimiz xidmətlər",
   services:"Xidmətlər",
   partners:"Tərəfdaşlar",
  blog:"Blog",
  projects:"Layihələr",
  clients:"MÜŞTƏRİLƏR",
  team_size:"Komanda üzvləri",
  years_on_market:"İllərdir bazarda",
  project_support:"Layihə dəstəyinə ehtiyacınız varmı?",
  call_us:"Bizə zəng edin və biz sizə ixtisaslı konsultasiya təqdim edək.",
  add_file:"Fayl əlavə edin",
  name_surname:"Ad Soyad",
  phone:"Telefon",
  email:"Email",
  send:"Göndər",
  our_address:"Mərkəzi Bulvar küç, Bakı, Azərbaycan",
  form_info:"Sorğunuzla bağlı sizinlə daha sonra əlaqə saxlamaq üçün şəxsi məlumatlarınız məxfilik siyasətimizə uyğun olaraq işlənəcək.",
  projects_we_provide:"Təqdim etdiyimiz layihələr",
  project_profile:"Layihə profili",
  about:"Haqqımızda",
  about_info:"Maestro Studio müştərilərinənikal və effektiv biznes həlləri təqdim edən kouçinq və biznes məsləhətləri studiyasıdır. Biz,müştərilərimizin layihələrinə sürət və keyfiyyət əsasında yanaşırıq və onları hər addımda dəstəkləyirik. Maestro Studio olaraq veb saytların və mobil tətbiqlərin yaradılmasında da ixtisaslaşmışıq. Bizim mütəxəssislərimiz, müştərilərimizin bütün digər layihələrində olduğu kimi, saytlarını və mobil tətbiqlərini də ən yüksək keyfiyyət standartları ilə hazırlayır.",
  your_message:"məktub"





}
